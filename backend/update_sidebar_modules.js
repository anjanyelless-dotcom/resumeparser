...
async function main() {
  ...
  try {
    ...
    for (const p of parents) {
      let res = await client.query('SELECT id FROM modules WHERE name = $1', [p.name]);
      if (res.rows.length === 0) {
        const id = crypto.randomUUID();
        await client.query(
          `INSERT INTO modules (id, name, display_name, sort_order, category) VALUES ($1, $2, $3, $4, $5)`,
          [id, p.name, p.display_name, p.sort_order, 'Dashboard Section']
        );
        const roleRes = await client.query("SELECT id FROM roles WHERE name = 'admin'");
        if (roleRes.rows.length > 0) {
          const permId = crypto.randomUUID();
          await client.query(
            "INSERT INTO role_permissions (id, role_id, module_id, action, allowed, sidebar_visible) VALUES ($1, $2, $3, $4, $5, $6) ON CONFLICT DO NOTHING",
            [permId, roleRes.rows[0].id, id, 'view', true, true]
          );
        }
        parentIds[p.name] = id;
      } else {
        parentIds[p.name] = res.rows[0].id;
        // ensure name and display_name are correct
        await client.query(
          `UPDATE modules SET display_name = $1, sort_order = $2, parent_module_id = NULL WHERE id = $3`,
    ...
    let dashboardId;
    let dashRes = await client.query("SELECT id FROM modules WHERE name = 'dashboard_parent'");
    if (dashRes.rows.length === 0) {
      dashboardId = crypto.randomUUID();
      await client.query(
        `INSERT INTO modules (id, name, display_name, sort_order, route, icon) VALUES ($1, $2, $3, $4, $5, $6)`,
        [dashboardId, 'dashboard_parent', 'Dashboard', 5, '/dashboard', 'dashboard']
      );
    } else {
      dashboardId = dashRes.rows[0].id;
      await client.query(
        `UPDATE modules SET display_name = 'Dashboard', sort_order = 5, parent_module_id = NULL, route = '/dashboard', icon = 'dashboard' WHERE id = $1`,
        [dashboardId]
      );
    }

    // Define mapping of existing/new children to their parents
    // We will update existing modules where possible, and insert new ones where they don't exist.
    // Ensure you provide: name, display_name, route, parent_name, sort_order
    const children = [
    ...
    for (const c of children) {
      let res = await client.query('SELECT id FROM modules WHERE name = $1', [c.name]);
      ...
      if (res.rows.length === 0) {
        ...
        await client.query(
          `INSERT INTO modules (id, name, display_name, sort_order, route, parent_module_id) VALUES ($1, $2, $3, $4, $5, $6)`,
          [childId, c.name, c.display_name, c.sort, c.route, parentId]
        );
      } else {
        childId = res.rows[0].id;
        await client.query(
          `UPDATE modules SET display_name = $1, sort_order = $2, route = $3, parent_module_id = $4 WHERE id = $5`,
          [c.display_name, c.sort, c.route, parentId, childId]
        );
      }

      // Ensure Admin can view the new modules
      const permId = crypto.randomUUID();
      await client.query(
        "INSERT INTO role_permissions (id, role_id, module_id, action, allowed, sidebar_visible) VALUES ($1, $2, $3, $4, $5, $6) ON CONFLICT DO NOTHING",
        [permId, adminRoleId, childId, 'view', true, true]
      );
    }

    await client.query('COMMIT');
    console.log('Sidebar modules successfully updated to match the Enterprise ATS Dashboard requested structure.');
  } catch (error) {
